...
ggplot(t0, aes(x=depth,y=value,fill=onto))+
  ...
  theme( axis.line.x=element_line(size=0.5*k, colour="black"),
         ...
         plot.title = element_text(family ="sans",size=8,hjust = 0.5,face="bold",margin = margin(5,0,40,0)))
...
ChaussabelPlot <- boxPlotFunction(chau.stat, ylim = c(0,500),"(A)") 
dataset = "BTM"
...
BTMPlot <- boxPlotFunction(stat, ylim = c(0,500),"(B)")
